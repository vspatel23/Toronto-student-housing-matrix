...
Exactly one active seed fixture intentionally demonstrates that state: `listing-021`, **Affordable Scarborough Basement**, omits address, coordinates, and stored commute estimates. The other 26 active seed listings have sufficient location data to produce an estimate for all six supported campuses. `listing-024`, **Etobicoke Shared House Room**, uses the corrected approximate location **Mabelle Avenue near Islington Station, Toronto, ON** at `43.6463, -79.5254`.

...
test("all active seed listings except listing-021 have a commute for all six campuses", () => {
  const activeListings = listings.filter((listing) => listing.isActive === true);
  const intentionalUnavailable = activeListings.find(
    (listing) => listing.seedId === "listing-021",
  );

  assert.equal(activeListings.length, 27);
  assert.equal(SUPPORTED_CAMPUS_LABELS.length, 6);
  assert.equal(intentionalUnavailable.title, "Affordable Scarborough Basement");
  assert.equal(intentionalUnavailable.location, undefined);
  assert.equal(intentionalUnavailable.commuteEstimates, undefined);
  ...
  SUPPORTED_CAMPUS_LABELS.forEach((campus) => {
    ...
    assert.equal(
      ...
      26,
      `${campus} should have 26 available estimates`,
    );
    assert.deepEqual(
      unavailableListings.map((listing) => listing.seedId),
      ["listing-021"],
      `${campus} should only leave listing-021 unavailable`,
    );
  });
});
